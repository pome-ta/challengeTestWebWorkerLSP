...
class LspServer {
	...
	async #recreateEnv() {
		...
		try {
			...
			for (const [path, content] of Object.entries(initialFiles)) {
				try {
					if (
						this.#env.getSourceFile &&
						this.#env.getSourceFile(path)
					) {
					...
					postLog(
						`recreateEnv sync failed for ${path}: ${
							e?.message ?? String(e)
						}`,
					);
				}
			...
			try {
				...
				postLog(
					`getProgram() during recreateEnv failed: ${
						e?.message ?? String(e)
					}`,
				);
			}
	...
	#mapTsDiagnosticToLsp(diag, path, program) {
		...
		try {
				...
				for (const ri of diag.relatedInformation) {
					try {
						...
						if (
							...
							riUri = `file://${
								ri.file.startsWith('/') ? ri.file : ri.file
							}`;
							// cannot compute line/char without SourceFile
						}

						const riMsg = ts.flattenDiagnosticMessageText(
							ri.messageText,
							'\n',
						);
						...
						postLog(
							`map relatedInformation error: ${String(
								e?.message ?? e,
							)}`,
						);
			...
			postLog(
				`relatedInformation mapping failed: ${String(e?.message ?? e)}`,
			);
		}
	...
	async publishDiagnostics(uri) {
		...
		try {
			...
			postLog(
				`getProgram() failed before diagnostics: ${
					e?.message ?? String(e)
				}`,
			);
		}
		...
		if (all.length > 0) {
			...
			for (const d of all) {
				try {
					...
					postLog(
						`  - (failed to stringify diag) ${String(
							e?.message ?? e,
						)}`,
					);
				}
