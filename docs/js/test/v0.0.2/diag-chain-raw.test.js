...
(async () => {
  ...
  try {
    ...
    const published = await waitForNotification(worker, 'textDocument/publishDiagnostics', (params) => params.uri === fileUri);
    ...
    const rawResult = await sendRequest(worker, 'lsp/_getRawDiagnostics', { uri: fileUri });
    ...
    const isChainObject = typeof rawDiag.messageText === 'object' && rawDiag.messageText !== null && ('messageText' in rawDiag.messageText);
    ...
    if (isChainObject) {
      ...
      expect(publishedMessage).to.include('Type').and.to.include('not assignable');
    }
